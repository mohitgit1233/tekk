  ...
  useEffect(() =>{
    ...
    if(userType != '') {
      const user_Google_Login = async () => {
        await fetch('http://10.128.200.42:5001/api/v1/google-user-login',{
          method: 'POST',
  ...
  useEffect(() => {
    ...
    if(googleAuthentication){
      const googleLogin = async () => {
        await fetch('http://10.128.200.42:5001/api/v1/google-login', {
          method: 'POST',
          headers: {
  ...
  const handleSignIn = async (e) => {
    ...
    await signIn(email,password).then(async (userCredentails) => {
      await userCredentails.user.getIdToken().then(async token=>{
          await fetch('http://10.128.200.42:5001/api/v1/login',{
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Accept: 'application/json', 'authorization': `Bearer ${token}` },
            body: JSON.stringify({})
          }).then((resp) => resp.json())
