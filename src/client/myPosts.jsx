...
const MyPosts = ({ route }) => {
  ...
  if (data.length > 0){
    if (postStatus == 'all'){
      //changed here
        filteredData = !data && data.filter(post => post.status !== 'offered' ).sort((a, b) => new Date(b.posted_date) - new Date(a.posted_date));
    } else{
        filteredData = !data && data.filter(post => post && post.status === postStatus && post.title.toLowerCase().includes(searchTerm.toLowerCase())).sort((a, b) => new Date(b.posted_date) - new Date(a.posted_date));
    }
}
